...
export default function stydux(mapStateToStyles, mergeStyles, options = {}) {
  ...
  return function wrapWithStydux(WrappedComponent) {
    ...
    class StyduxComponent extends Component {
      ...
      updateMappedStylesIfNeeded() {
        ...
        const nextMappedStyles = this.computeMappedStyles(styles, this.props)
      ...
      trySubscribe() {
        if (shouldSubscribe && !this.unsubscribe) {
          this.unsubscribe = this.stydux.subscribe(this.handleChange.bind(this))
          this.handleChange()
        }
      }
